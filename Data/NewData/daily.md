# Daily Data Processing and Handling Missing Data

## Overview

This document provides a detailed explanation of the data processing and missing data handling steps performed by the `dailyScriptWithCombinedData.py` script. The script processes daily data from multiple sources, calculates additional attributes, and ensures data completeness by handling missing values.
...
- **Output File**: `/Users/bibekg/Documents/Learning/blockchain/AQI_COL/Air-Quality-Prediction/airprediction/Data/NewData/filtered_data_daily.csv`
- **PM2.5 Input Files**:
...
The average wind speed in miles per hour for each month is as follows:
...
A dictionary is initialized for all days in the range from January 1, 2021, to December 31, 2023, with each parameter set to an empty list.
...
For each date, the average value for each parameter is calculated if there are values available. Missing values are initially set to `None`.
...
### 7. Adjust Wind Speed and Temperature

- **Wind Speed (WND)**: Ensure that the wind speed (`WND`) is not greater than the wind gust observation (`OC1`). Adjust wind speed to the average if necessary.
- **Temperature (TMP)**: Calculate the average temperature (`TMP`) as the average of the maximum temperature (`KA1`) and the minimum temperature (`OC1`).

### 8. Write to Output CSV

The aggregated and purified data is written to the output CSV file.

### 9. Re-read and Fill Remaining Missing Values

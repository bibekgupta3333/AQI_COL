# Hourly Data Processing and Handling Missing Data

## Overview

This document provides a detailed explanation of the data processing and missing data handling steps performed by the `hourlyScriptWithCombinedData.py` script. The script processes hourly data from multiple sources, calculates additional attributes, and ensures data completeness by handling missing values.

## Input and Output Files

- **Input File**: `/Users/bibekg/Documents/Learning/blockchain/AQI_COL/Air-Quality-Prediction/airprediction/Data/NewData/source.csv`
- **Output File**: `/Users/bibekg/Documents/Learning/blockchain/AQI_COL/Air-Quality-Prediction/airprediction/Data/NewData/filtered_data_hourly.csv`
- **PM2.5 Input Files**:
...
The following parameters are filtered and processed:
...
The average monthly humidity percentages are used to adjust the calculated Relative Humidity (RH):
...
The average wind speed in miles per hour for each month is converted to kilometers per hour:
...
The script includes several conversion functions to process raw data values:

- `convert_dew_point(value)`: Converts dew point temperature.
- `convert_extreme_air_temp(value)`: Converts extreme air temperature.
- `convert_wind_gust(value)`: Converts wind gust observation.
- `convert_sea_level_pressure(value)`: Converts sea level pressure.
- `convert_dry_bulb_temp(value)`: Converts dry bulb temperature.
- `convert_visibility(value)`: Converts visibility observation.
- `convert_wind_observation(value)`: Converts wind observation.

## Relative Humidity Calculation

The script calculates Relative Humidity (RH) using the following formula:
\[
RH = 100 \times \frac{e*{dew}}{e*{temp}}
\]
Where:

- \( e\_{temp} \): Saturation vapor pressure at the Dry Bulb Temperature (TMP).
- \( e\_{dew} \): Saturation vapor pressure at the Dew Point Temperature (DEW).

The saturation vapor pressure is calculated using the Magnus-Tetens approximation:
\[
e = 6.112 \times \exp\left(\frac{17.62 \times T}{T + 243.12}\right)
\]
Where:

- \( T \): Temperature in degrees Celsius (\(^\circ C\)).
- \( e \): Saturation vapor pressure in hPa.

## Steps

### 1. Initialize Date Dictionary

A dictionary is initialized for all hours in the range from January 1, 2021, to December 31, 2024, with each parameter set to an empty list.

### 2. Read PM2.5 CSV Files

The script reads the PM2.5 CSV files and extracts AQI values, which are then stored in the date dictionary. Negative AQI values (`-999` and `-312.33`) are ignored.

### 3. Process Input CSV

Each row in the input CSV file is processed to extract the date and parameter values. The values are converted using the conversion functions and stored in the dictionary.

### 4. Aggregate Data

For each hour, the average value for each parameter is calculated if there are values available. Missing values are initially set to `None`.

### 5. Calculate Relative Humidity

Relative Humidity (RH) is calculated based on Dew Point Temperature (DEW) and Dry Bulb Temperature (TMP) and adjusted according to the average monthly humidity percentages.

### 6. Handle Missing Values

Missing values in the current row are filled based on the previous row's data. If the previous row also has missing values, the same data from the previous year or the latest year date is used.

### 7. Write to Output CSV

The aggregated and purified data is written to the output CSV file.

### 8. Re-read and Fill Remaining Missing Values

The output CSV file is re-read, and any remaining missing values are filled based on the previous row's data or the same data from the previous year or latest year date.

### 9. Rename Headers

The headers in the final output file are renamed as specified:
...
output_file = "/Users/bibekg/Documents/Learning/blockchain/AQI_COL/Air-Quality-Prediction/airprediction/Data/NewData/filtered_data_hourly.csv"
...
def convert_dew_point(value):
    return round(int(value[:4]) / 10.0, 2)


def convert_extreme_air_temp(value):
    return round(int(value.split(",")[2]) / 10.0, 2)


def convert_wind_gust(value):
    return round(int(value.split(",")[0]) / 10.0, 2)
...
def convert_visibility(value):
    return round(int(value[:6]) / 1000.0, 2)
...
def calculate_relative_humidity(dew_point, dry_bulb_temp, month):
    ...
    return round(rh_adjusted, 2)


# Initialize the dictionary for all hours in the range from 2021 to 2024
date_rows = {}
...
with open(output_file, mode="w", newline="") as outfile:
    ...
    for date_str, aggregated_row in date_rows.items():
        ...
        if aggregated_row["DEW"] is not None and aggregated_row["TMP"] is not None:
            month = datetime.strptime(date_str, "%Y-%m-%dT%H:00:00").month
            aggregated_row["RH"] = calculate_relative_humidity(
        ...
        if date_rows[date_str]["AQI"]:
            ...
            for year_offset in [-1, 1]:
                other_year_date_str = (
                    datetime.strptime(date_str, "%Y-%m-%dT%H:00:00")
...
with open(output_file, mode="w", newline="") as outfile:
    ...
    for row in reader:
        for param in parameters:
            if row[param] == "" or row[param] is None:
                if previous_row and previous_row[param] != "":
                    ...
                    date_str = row["Date"]
                    date = datetime.strptime(date_str, "%Y-%m-%dT%H:00:00")
        ...
        # Adjust wind speed and temperature
        month = datetime.strptime(row["Date"], "%Y-%m-%dT%H:00:00").month
        average_wind_speed_kph = (
            average_wind_speed_mph[month] * 1.60934
        )  # Convert to km/h
        ...
        for param, value in row.items():
            if param == "Date":
                new_row[new_headers.get(param, param)] = value
            else:
                new_row[new_headers.get(param, param)] = (
                    round(float(value), 2) if value != "" else value
                )
